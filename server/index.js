...
class Room {
    constructor(players) {
        ...
        io.to(this.room_id).emit("room_created", { room_id: this.room_id, players: this.player_id });
    ...
    leaveRoom = (player) => {
        ...
        io.to(this.room_id).emit("transmit_info_to_room", { player_id: player.id, health: 0, wpm: 0 });
    }
    sendAttack = (player_id, type) =>  {
        const wordbank = (type === "scientific" ? scientific : (type === "spanish" ? spanish : gibberish));
        assert(wordbank.words !== null) 

        io.to(this.room_id).emit("player_attacked", { player_id: player_id, words: wordbank.getBatch() })
    }

    queueWords = () => {
        if (Date.now() - this.last_queue_words < 5000)
            return;
